...
Status DeserializeDict(std::shared_ptr<Array> array, int32_t start_idx, int32_t stop_idx, PyObject** out) {
  ...
  static PyObject* py_ray_id = PyString_FromString("__ray_id__");
  if (PyDict_Contains(result, py_ray_id) && ray_deserialize_callback) {
    PyObject* arglist = Py_BuildValue("(O)", result);
    result = PyObject_CallObject(ray_deserialize_callback, arglist);
    if (!result) {
      return python_error_to_status();
    }
    Py_XDECREF(arglist);
